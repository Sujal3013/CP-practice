...
void printBinary(int num){for(int i=25;i>=0;--i){cout<<((num>>i)&1);}endl;}

ll finds(int low,int high,int d){
    return (high-low-1)/d;
}
...
void solve(){
  ...
  pref[0]=1+(a[0]+d-2)/d;
  f(i,1,m){
    pref[i]=1+pref[i-1]+finds(a[i-1],a[i],d);
  }
  suff[m-1]=1+finds(a[m-1],n+1,d);

  for(int i=m-2;i>=0;i--){
    suff[i]=1+suff[i+1]+finds(a[i],a[i+1],d);
  }
  ...
  mp[ans]++;
  {
    int curr=pref[m-2]+finds(a[m-2],n+1,d);
    if(ans>curr){
        ans=curr;
        removed=m-1;
    }
  ...
  f(i,1,m-1){
    int curr=pref[i-1]+suff[i+1]+finds(a[i-1],a[i+1],d);
    if(curr<ans){
        ans=curr;
        removed=i;
    }
