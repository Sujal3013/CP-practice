...
*/
pair<const int, const int> modulo = {1e9 + 7, 1e9 + 9};
pair<const int, const int> poly = {31, 53};
void solve()
{
    ...
    cin >> s;
    int n = s.size();
    vector<pair<int,int>> prefix_hash(n), poly_power(n);
    prefix_hash[0] = {s[0] - 'a' + 1, s[0] - 'a' + 1};
    poly_power[0] = {1, 1};

    for (int i = 1; i < n; i++)
    {
        prefix_hash[i].first = (prefix_hash[i - 1].first * poly.first + (s[i] - 'a' + 1)) % modulo.first;
        prefix_hash[i].second = (prefix_hash[i - 1].second * poly.second + (s[i] - 'a' + 1)) % modulo.second;

        poly_power[i].first = (poly_power[i - 1].first * poly.first) % modulo.first;
        poly_power[i].second = (poly_power[i - 1].second * poly.second) % modulo.second;
    }

    for (int i = 0; i < n - 1; i++)
    {
        pair<int, int> hash_p = prefix_hash[i], hash_s;
        hash_s.first = prefix_hash[n - 1].first;
        hash_s.second = prefix_hash[n - 1].second;

        hash_s.first = ((hash_s.first - prefix_hash[n - 1 - (i + 1)].first * poly_power[(i + 1)].first) % modulo.first+modulo.first)%modulo.first;
        hash_s.second = ((hash_s.second - prefix_hash[n - 1 - (i + 1)].second * poly_power[i + 1].second) % modulo.second+modulo.second)%modulo.second;
